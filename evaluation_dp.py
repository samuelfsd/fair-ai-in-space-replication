...
# ---------------------------------------------------------------------
# Global Variable
# 1. APONTAR PARA A PASTA DP
model_dir = 'results_dp'
dir_ckpt = model_dir + '/' + 'checkpoints'

MAX_ROW_PARTITION = 5
MAX_COL_PARTITION = 5

BATCH_SIZE = 250 # Batch size consistente com o treino DP

INPUT_SIZE = 10  # number of features
NUM_LAYERS_DNN = 8  # num_layers is the number of non-input layers, one more LSTM
NUM_CLASS = 23
EPOCH_TRAIN = 3 # Consistente com o treino DP
...
# ---------------------------------------------------------------------
# DNN model (Modelo retorna LOGITS)
class DenseNet(tf.keras.Model):
    ...
    def call(self, inputs):  # , num_layers = NUM_LAYERS_DNN
        ...
        out_layer = self.out(layer) # Retorna LOGITS
...
# 2. FUNÇÃO DE LOSS ATUALIZADA (apenas para compilar)
loss_fn = tf.keras.losses.CategoricalCrossentropy(
    from_logits=True, reduction=tf.losses.Reduction.NONE)

# Otimizador: pode ser qualquer um, já que não vamos treinar
# (mas usar o legacy Adam é uma boa prática se o Adam original der erro de compatibilidade)
...
# ---------------------------------------------------------------------
# 3. FUNÇÕES DE MÉTRICA ATUALIZADAS (para lidar com logits)

def get_class_wise_accuracy(y_true, y_pred, prf=False):
    num_class = y_true.shape[1]
    y_pred_softmax = tf.nn.softmax(y_pred) # Aplicar Softmax
    stat = tf.keras.metrics.categorical_accuracy(y_true, y_pred_softmax)
    ...
    if prf:
        pred_w_class = tf.math.argmax(y_pred_softmax, axis=1)
...
def get_fairness_loss_all(y_test, y_pred, partition_list, all_partitioning_data_list):
    ...
    true_pred_w_class, pred_w_class = get_class_wise_list(y_test, y_pred) # Já lida com logits
