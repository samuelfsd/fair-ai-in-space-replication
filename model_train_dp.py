...
# removi para usar a do DP, função de perda
...
class DenseNet(tf.keras.Model):
    ...
    def __init__(self, layer_size=INPUT_SIZE, num_class=NUM_CLASS, ckpt_path=CKPT_FOLDER_PATH):
        ...
        initializer = tf.keras.initializers.TruncatedNormal(stddev=0.5, seed=42)  # mean=0.0, seed=None
        self.dense = []
        for i in range(7):
...
def model_train(model, X_in, y_in, init_epoch_number=0, epoch_train=EPOCH_TRAIN):  #
    ...
    Input model is complied!
    '''
    train_dataset = tf.data.Dataset.from_tensor_slices((X_in, y_in))

    train_dataset = train_dataset.shuffle(buffer_size=len(X_in)).batch(BATCH_SIZE, drop_remainder=True)
...
loss = loss_fn(y.astype('float32'), y_pred).numpy()
        ...
        for j in range(len(PARTITIONINGS)):
            ...
            for e in range(epochs):
                for p in range(index1 * index2):
                    K.set_value(optimizer.learning_rate, lr_list[p])

                    print(f"Loop {l+1}/{loops}, Part {j+1}/{len(PARTITIONINGS)}, Epoch {e+1}/{epochs}, Sub-Part {p+1}/{index1*index2}, LR: {lr_list[p]}")
